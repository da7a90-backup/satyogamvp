  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto">
        ...
        {/* Desktop: Grid of 3, Mobile: Vertical List */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          {data.cards.map((card, index) => (
            <RetreatCardGrid key={index} card={card} />
          ))}
        </div>
      </div>
    </section>
...
// Retreat Card Component - Grid format (vertical layout for grid)
const RetreatCardGrid = ({ card }: { card: Card }) => {
  return (
    <div className="flex flex-col bg-white rounded-lg overflow-hidden border border-gray-200">
      {/* Image */}
      <div className="relative w-full" style={{ aspectRatio: '4/3', backgroundColor: '#f3f4f6' }}>
        <img
          src={card.image}
          alt={card.title}
...
// Retreat Card Component (smaller list format - kept for backward compatibility)
