...
class NewsletterSubscriber(Base):
    ...
    subscriber_metadata = Column(JSON_TYPE, nullable=True, default={})
...
class EmailTemplate(Base):
    ...
    beefree_json = Column(JSON_TYPE, nullable=True)  # Beefree template JSON
    html_content = Column(Text, nullable=False)  # rendered HTML
    variables = Column(JSON_TYPE, nullable=True, default=[])  # list of template variables like {{name}}, {{email}}
...
class EmailCampaign(Base):
    ...
    id = Column(UUID_TYPE, primary_key=True, default=uuid.uuid4, index=True)
    name = Column(String(255), nullable=False)
    template_id = Column(UUID_TYPE, ForeignKey("email_templates.id", ondelete="CASCADE"), nullable=False)
    subject = Column(String(500), nullable=False)
    from_name = Column(String(255), nullable=False)
    from_email = Column(String(255), nullable=False)
    segment_filter = Column(JSON_TYPE, nullable=True)  # conditions for filtering subscribers
...
class EmailAutomation(Base):
    ...
    trigger_config = Column(JSON_TYPE, nullable=False)  # e.g., {"event": "user_signup", "properties": {...}}
    template_id = Column(UUID_TYPE, ForeignKey("email_templates.id", ondelete="CASCADE"), nullable=False)
...
class EmailSent(Base):
    ...
    id = Column(UUID_TYPE, primary_key=True, default=uuid.uuid4, index=True)
    campaign_id = Column(UUID_TYPE, ForeignKey("email_campaigns.id", ondelete="CASCADE"), nullable=True, index=True)
    automation_id = Column(UUID_TYPE, ForeignKey("email_automations.id", ondelete="CASCADE"), nullable=True, index=True)
    subscriber_id = Column(UUID_TYPE, ForeignKey("newsletter_subscribers.id", ondelete="CASCADE"), nullable=False, index=True)
    template_id = Column(UUID_TYPE, ForeignKey("email_templates.id", ondelete="CASCADE"), nullable=False)
