...
import RetreatsPageClient from "@/components/dashboard/retreat/RetreatsPageClient";

// Fetch user's registered retreats
async function getUserRetreats(accessToken: string) {
  try {
    ...
    // Each registration has a nested retreat object
    const retreats = (data.registrations || []).map((reg: any) => ({
      ...reg.retreat,
    ...
    console.error('Error fetching user retreats:', error);
    return [];
  }
}

// Fetch all available online retreats
async function getAvailableRetreats(accessToken?: string) {
  try {
    const API_BASE_URL = process.env.NEXT_PUBLIC_FASTAPI_URL || 'http://localhost:8000';
...
export default async function RetreatsPage() {
    ...
    return (
      <div className="min-h-screen bg-[#FAF8F1] p-8">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-4xl font-bold text-[#000000] mb-4">Online Retreats</h1>
  ...
  const [userRetreats, availableRetreats] = await Promise.all([
    getUserRetreats(accessToken),
    getAvailableRetreats(accessToken),
  ]);

  return (
    <RetreatsPageClient
      initialUserRetreats={userRetreats}
      initialAvailableRetreats={availableRetreats}
    />
  );
}
